...
BORG_Problem BORG_Problem_create(
  ...
  BORG_Validate_positive(numberOfInfo);  
  ...
  problem->numberOfInfo = numberOfInfo;
...
int BORG_Problem_number_of_info(BORG_Problem problem) {
  BORG_Validate_pointer(problem);
	return problem->numberOfInfo;
}
...
BORG_Solution BORG_Solution_create(BORG_Problem problem) {
  ...
  solution->info = (double*)calloc(problem->numberOfInfo, sizeof(double));
	BORG_Validate_malloc(solution->info);
...
BORG_Solution BORG_Solution_clone(BORG_Solution original) {
  ...
  memcpy(clone->info, original->info, problem->numberOfInfo * sizeof(double));
...
double BORG_Solution_get_info(BORG_Solution solution, int index) {
  BORG_Validate_pointer(solution);
	BORG_Validate_index(index, solution->problem->numberOfInfo);

	return solution->info[index];
}
...
void BORG_Solution_set_info(BORG_Solution solution, int index, double value) {
  BORG_Validate_pointer(solution);
	BORG_Validate_index(index, solution->problem->numberOfInfo);

	solution->info[index] = value;
}
...
void BORG_Solution_evaluate(BORG_Solution solution) {
  ...
  //int numberOfinfo = problem->numberOfInfo;  
	int numberOfConstraints = problem->numberOfConstraints;

	solution->problem->function(solution->variables, solution->objectives, solution->info, solution->constraints);
...
void BORG_Solution_print(BORG_Solution solution, FILE* fp) {
  ...
  for (i=0; i<solution->problem->numberOfInfo; i++) {
		fprintf(fp, " %.*g", BORG_DIGITS, solution->info[i]);
	}
...
void BORG_Archive_append(BORG_Archive archive, FILE* file) {
  int i;
	...
	for (i=0; i<BORG_Archive_get_size(archive); i++) {
  	...
  	for (j=0; j<solution->problem->numberOfInfo; j++) {
			if (j > 0 || solution->problem->numberOfVariables > 0) {
				fprintf(file, " ");
			}

			fprintf(file, "%.*g", BORG_DIGITS, solution->info[j]);
		}
...
void BORG_Algorithm_checkpoint(BORG_Algorithm algorithm, FILE *file) {
  ...
  fprintf(file, "Number of Info: %d\n", problem->numberOfInfo);
	...
	for (i = 0; i < population->size; i++) {
  	...
  	for (j = 0; j < problem->numberOfInfo; j++) {
			fprintf(file, " %.*g", BORG_DIGITS, solution->info[j]);
		}
...
void BORG_Algorithm_restore(BORG_Algorithm algorithm, FILE *file) {
  ...
  BORG_Check_scan1(fscanf(file, "Number of Info: %d\n", &readInt));

	if (readInt != problem->numberOfInfo) {
		BORG_Error("Checkpoint file does not match number of info\n");
	}
	...
	for (i = 0; i < populationSize; i++) {
  	...
  	for (j = 0; j < problem->numberOfInfo; j++) {
			BORG_Check_scan1(fscanf(file, " %lg", &solution->info[j]));
		}
...
void ICOW_Solution_print(BORG_Solution solution, FILE* fp) {
  BORG_Validate_pointer(solution);
  BORG_Validate_pointer(fp);
  ...
  CharacterizeCity (solution->variables[0],solution->variables[3],solution->variables[1],solution->variables[2],solution->variables[4],city);
  fprintf(fp, " %.*g", BORG_DIGITS, city[wc]);
  fprintf(fp, " %.*g", BORG_DIGITS, city[rc]);
  fprintf(fp, " %.*g", BORG_DIGITS, city[dc]);
  fprintf(fp, " %.*g", BORG_DIGITS, city[tic]);
  fprintf(fp, " %.*g", BORG_DIGITS, city[wh]);
  fprintf(fp, " %.*g", BORG_DIGITS, city[rh]);
  fprintf(fp, " %.*g", BORG_DIGITS, city[rp]);
  fprintf(fp, " %.*g", BORG_DIGITS, city[dbh]);
  fprintf(fp, " %.*g", BORG_DIGITS, city[dh]);
  */
  fprintf(fp, "\n");}

void ICOW_Archive_print(BORG_Archive archive, FILE* fp) {
  BORG_Validate_pointer(archive);
  BORG_Validate_pointer(fp);
	...
	while (entry) {
		ICOW_Solution_print(entry->solution, fp);
		entry = entry->next;
	}
}
