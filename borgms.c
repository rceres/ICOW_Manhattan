...
int BORG_Master_receive() {
  ...
  int ninfo = problem->numberOfInfo;
	int ncons = problem->numberOfConstraints;
	MPI_Status status;
	double* message = (double*)calloc(nobjs+ninfo+ncons, sizeof(double));

	BORG_Validate_malloc(message);

	rc = MPI_Recv(message, nobjs+ninfo+ncons, MPI_DOUBLE, MPI_ANY_SOURCE,
  ...
  //memcpy (destination, const void * source, size_t num );
	memcpy(solution->objectives, message, nobjs*sizeof(double));
  if (ninfo > 0) {
		memcpy(solution->info, message+nobjs, ninfo*sizeof(double));
	}
	if (ncons > 0) {
		memcpy(solution->constraints, message+nobjs+ninfo, ncons*sizeof(double));
	}
...
void BORG_Master_output_header(BORG_Algorithm algorithm, FILE* file) {
	...
	if (aerovisFormat) {
		...
		fprintf(file, "# Format:  Variables = %d | Objectives = %d | Info = %d\n",
				problem->numberOfVariables, problem->numberOfObjectives,problem->numberOfInfo);
  	...
  	for (i=0; i<problem->numberOfInfo; i++) {
			if (i > 0) {
				fprintf(file, ", ");
			}

			fprintf(file, "DI%d", i+1);
		}
...
void BORG_Worker_run() {
  ...
  int ninfo = problem->numberOfInfo;  
	...
	double* output = (double*)calloc(nobjs+ninfo+ncons, sizeof(double));
	...
	while (1) {
		...
		if (rc != MPI_SUCCESS) {
			...
			problem->function(input, output, output+nobjs, ncons == 0 ? NULL : output+nobjs+ninfo);
			BORG_Timer_stop("eval");

			BORG_Timer_start("send");
			rc = MPI_Send(output, nobjs+ninfo+ncons, MPI_DOUBLE, 0, TAG_RESULT, MPI_COMM_WORLD);
			BORG_Timer_stop("send");

			if (rc != MPI_SUCCESS) {
				fprintf(stderr, "MPI_Send: %s failed to send objectives, info, and constraints\n", name);
				MPI_Abort(MPI_COMM_WORLD, rc);
			}
		}
	}
